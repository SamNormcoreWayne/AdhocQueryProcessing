...
class operateClass():
	...
	def __init__(self, selectAttr : str, havingCond : str, size : int, group_attr, agg_func, select_cond):
		...
		self.select_conds = select_cond
		self.part_size = size

	def parse_agg_group(self):
		for var_name, func_name in self.agg_func.items():
	...
	def parse_agg_func(unparsed_agg_func):
		...
		'''
		parsed_func = []
		for item in unparsed_agg_func:
			fuc_lst = item.split('_')
			if len(fuc_lst) == 3:
